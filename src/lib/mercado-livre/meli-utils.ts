...
import type { MeliAlertType, MeliRuptureRisk } from '@prisma/client';
import type { MeliProductData, MeliProductSettingsType, MeliSalesHistoryType } from './meli-types';
...
function calculateDynamicSuggestedPricing(
  ...
  if (type === ('DEAD_STOCK' as unknown as MeliAlertType)) {
...
export function calculateDaysSinceLastSale(orders: MeliSalesHistoryType[]): number;
...
export function calculateDaysSinceLastSale(
  ordersOrDate: MeliSalesHistoryType[] | Date | null,
...
export function calculateDaysOutOfStock(
  ordersOrDate: MeliSalesHistoryType[] | Date,
...
export function determineProductType(
  ...
  if (
    ...
    return 'LIQUIDATION' as unknown as MeliAlertType;
  }
  return 'FINE';
}
 ...
 */
export function mapAlertTypeToPtLabel(type: MeliAlertType): string {
  return getAlertTypeLabel(type as unknown as Parameters<typeof getAlertTypeLabel>[0]);
}
 ...
 */
export function getDaysWithSales(sales: MeliSalesHistoryType[]): number {
 ...
 */
export function getTotalSales(sales: MeliSalesHistoryType[]): number {
 ...
 */
export function getLastSaleDate(sales: MeliSalesHistoryType[]): Date | null {
 ...
 */
export function getTotalLastSales(sales: MeliSalesHistoryType[], days: number): number {
  const now = new Date();
...
export function calculateAllMetrics(
  ...
  const pd = productData as unknown as {
    orders30?: MeliSalesHistoryType[];
    orders7?: MeliSalesHistoryType[];
  };
